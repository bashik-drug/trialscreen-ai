...
function screenPatient(p) {
  const age         = Number(p.age);
  const ecog        = Number(p.ecog);
  const egfr        = Number(p.egfr);
  const alt         = Number(p.alt);
  const ast         = Number(p.ast);
  const lifeExp     = Number(p.lifeExpectancy);
  ...
  // ── Inclusion checks ──
  if (age >= 18 && age <= 75)           met.push("Age within range (18–75)");
  else                                   failed.push(`Age ${age} outside required range (18–75)`);

  const validStage = stage.includes("iiib") || stage.includes("3b") || stage.includes("iv") || stage.includes("4");
  if (validStage)                        met.push("NSCLC Stage IIIB/IV confirmed");
  else                                   failed.push("Stage does not meet IIIB/IV requirement");

  const validDx = diagnosis.includes("nsclc") || diagnosis.includes("non-small") || diagnosis.includes("lung");
  if (validDx)                           met.push("NSCLC diagnosis confirmed");
  else                                   failed.push("Diagnosis does not confirm NSCLC histology");

  if (ecog >= 0 && ecog <= 2)           met.push(`ECOG PS ${ecog} within acceptable range (0–2)`);
  else                                   failed.push(`ECOG PS ${ecog} exceeds maximum of 2`);

  if (egfr >= 50)                        met.push(`eGFR ${egfr} mL/min — adequate renal function`);
  else                                   failed.push(`eGFR ${egfr} mL/min below minimum (50 mL/min)`);

  if (alt <= 2.5)                        met.push(`ALT ${alt}× ULN — within hepatic limits`);
  else                                   failed.push(`ALT ${alt}× ULN exceeds limit of 2.5× ULN`);

  if (ast <= 2.5)                        met.push(`AST ${ast}× ULN — within hepatic limits`);
  else                                   failed.push(`AST ${ast}× ULN exceeds limit of 2.5× ULN`);

  if (lifeExp >= 12)                     met.push(`Life expectancy ${lifeExp} weeks meets minimum (≥12 weeks)`);
  else                                   failed.push(`Life expectancy ${lifeExp} weeks below minimum (12 weeks)`);

  // ── Exclusion checks ──
  if (p.priorChemo === "Yes")            failed.push("Prior platinum-based chemotherapy — exclusion criterion");
  else                                   met.push("No prior platinum-based chemotherapy");

  if (p.cnsMetastases === "Yes")         failed.push("Active untreated CNS metastases — exclusion criterion");
  else                                   met.push("No active CNS metastases");

  if (p.pregnant === "Yes")             failed.push("Pregnant or breastfeeding — exclusion criterion");
  else                                   met.push("Not pregnant or breastfeeding");

  if (p.autoimmune === "Yes")           failed.push("Active autoimmune disease requiring systemic treatment — exclusion criterion");
  else                                   met.push("No active autoimmune disease");

  // ── Verdict ──
  let verdict;
  if (failed.length === 0)              verdict = "ELIGIBLE";
  else if (failed.length <= 2 && failed.every(f => !f.includes("exclusion"))) verdict = "REQUIRES REVIEW";
  else                                   verdict = "NOT ELIGIBLE";

  // ── Dropout Risk ──
  let riskScore = 0;
  if (ecog === 2)                        riskScore += 2;
  if (ecog > 2)                          riskScore += 4;
  if (lifeExp < 20)                      riskScore += 2;
  if (age > 65)                          riskScore += 1;
  if (egfr < 60)                         riskScore += 1;

  const dropoutRisk       = riskScore >= 4 ? "HIGH" : riskScore >= 2 ? "MODERATE" : "LOW";
  const dropoutRiskReason = riskScore >= 4
    ? "Multiple clinical risk factors suggest elevated likelihood of early withdrawal or protocol deviation."
    : riskScore >= 2
    ? "Moderate performance or organ function concerns warrant closer monitoring for dropout."
    : "Patient profile indicates good tolerance and compliance likelihood.";

  // ── Recommendation ──
  let recommendation;
  if (verdict === "ELIGIBLE")
    recommendation = "Patient meets all inclusion criteria with no exclusion flags. Proceed to informed consent and baseline assessments. Schedule site visit within 14 days.";
  else if (verdict === "REQUIRES REVIEW")
    recommendation = `Patient narrowly misses ${failed.length} criterion/criteria. Recommend medical monitor review before final decision. Do not proceed without sign-off.`;
  else
    recommendation = `Patient fails ${failed.length} criteria and is not eligible for enrolment. Document screen failure in the screening log. Consider alternative trials if appropriate.`;
...
export default function App() {
  ...
  const scoredSites = scoreSites(SITES);

  const handleChange = e => { setPatient({ ...patient, [e.target.name]: e.target.value }); setResult(null); };

  const handleScreen = () => {
    const res   = screenPatient(patient);
    const entry = { ...res, patientId: patient.id || "PT-" + Date.now().toString().slice(-4), age: patient.age, diagnosis: patient.diagnosis };
    setResult(entry);
  ...
  const inputStyle = {
    width: "100%", background: "#0B0F1A", border: "1px solid #1E2535",
    borderRadius: 6, padding: "8px 12px", color: "#E8EAF0", fontSize: 13,
    outline: "none", boxSizing: "border-box",
  };
  const labelStyle = {
    fontSize: 11, color: "#5A6480", display: "block",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0B0F1A", fontFamily: "'Inter', sans-serif", color: "#E8EAF0" }}>
      ...
      {/* ── Protocol Banner ── */}
      <div style={{ background: "#111827", borderBottom: "1px solid #1E2535", padding: "10px 32px", display: "flex", gap: 24, alignItems: "center" }}>
        <span style={{ fontSize: 12, color: "#4F8EF7", fontWeight: 700 }}>{PROTOCOL.phase}</span>
        <span style={{ fontSize: 12, color: "#8892A4" }}>{PROTOCOL.title}</span>
        <span style={{ marginLeft: "auto", fontSize: 12, background: "#1E2535", padding: "3px 10px", borderRadius: 20, color: "#A259FF" }}>{PROTOCOL.indication}</span>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
            ...
            <div style={{ background: "#111827", borderRadius: 12, padding: 24, border: "1px solid #1E2535" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                {[
                  ...
                  { label:"Age (years)",            name:"age",            placeholder:"58",  type:"number" },
                  { label:"Diagnosis",              name:"diagnosis",      placeholder:"NSCLC Adenocarcinoma" },
                  { label:"Stage",                  name:"stage",          placeholder:"Stage IIIB or IV" },
                  { label:"ECOG Status (0–4)",      name:"ecog",           placeholder:"1",   type:"number" },
                  { label:"eGFR (mL/min)",          name:"egfr",           placeholder:"65",  type:"number" },
                  { label:"ALT (× ULN)",            name:"alt",            placeholder:"1.2" },
                  { label:"AST (× ULN)",            name:"ast",            placeholder:"1.0" },
                  { label:"Life Expectancy (weeks)",name:"lifeExpectancy", placeholder:"20",  type:"number" },
                ].map(f => (
                  <div key={f.name} style={{ gridColumn: ["diagnosis","lifeExpectancy"].includes(f.name) ? "span 2" : "span 1" }}>
                    <label style={labelStyle}>{f.label}</label>
                    <input name={f.name} value={patient[f.name]} onChange={handleChange}
                      placeholder={f.placeholder} type={f.type || "text"} style={inputStyle} />
                  </div>
                ))}
                {[
                  { label:"Prior Platinum Chemo?",      name:"priorChemo" },
                  { label:"Active CNS Metastases?",     name:"cnsMetastases" },
              ...
              <button onClick={handleScreen} disabled={!patient.age || !patient.diagnosis || !patient.stage || !patient.ecog || !patient.egfr}
                style={{
                  marginTop: 20, width: "100%", padding: 12, borderRadius: 8, border: "none",
                  cursor: (!patient.age || !patient.diagnosis) ? "not-allowed" : "pointer",
                  fontWeight: 700, fontSize: 14, letterSpacing: "0.04em",
                  background: (!patient.age || !patient.diagnosis) ? "#1E2535" : "linear-gradient(135deg,#4F8EF7 0%,#A259FF 100%)",
                  ...
                  {result.metCriteria.length > 0 && (
                    <div>
                      ...
                      {result.metCriteria.map((c,i) => (
                        <div key={i} style={{ fontSize: 12, color: "#8892A4", padding: "3px 0", display: "flex", gap: 8 }}>
                          <span style={{ color:"#00C48C" }}>✓</span>{c}
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {result.failedCriteria.length > 0 && (
                    <div>
                      ...
                      {result.failedCriteria.map((c,i) => (
                        <div key={i} style={{ fontSize: 12, color: "#8892A4", padding: "3px 0", display: "flex", gap: 8 }}>
                          <span style={{ color:"#FF4D6D" }}>✗</span>{c}
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {tab === "sites" && (
          <div style={{ background: "#111827", borderRadius: 12, padding: 24, border: "1px solid #1E2535" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#8892A4", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 6 }}>Site Feasibility Ranking</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {scoredSites.map((site,i) => (
                <div key={site.name} style={{ background: "#0B0F1A", borderRadius: 10, padding: "16px 20px", border: i===0 ? "1px solid #4F8EF7" : "1px solid #1E2535", position: "relative" }}>
                  {i===0 && <div style={{ position:"absolute", top:-1, right:16, background:"#4F8EF7", color:"#fff", fontSize:10, fontWeight:800, padding:"2px 10px", borderRadius:"0 0 6px 6px", letterSpacing:"0.06em" }}>TOP SITE</div>}
                  <div style={{ display:"flex", alignItems:"center", gap:16 }}>
                    <div style={{ width:36, height:36, borderRadius:8, background: i===0 ? "linear-gradient(135deg,#4F8EF7,#A259FF)" : "#1E2535", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:800, fontSize:14, color:"#fff", flexShrink:0 }}>{i+1}</div>
                    <div style={{ flex:1 }}>
                    ...
                    <div style={{ display:"flex", gap:20, alignItems:"center" }}>
                      ...
                      <div style={{ textAlign:"center", minWidth:60 }}>
                        <div style={{ fontSize:22, fontWeight:800, color: i===0 ? "#4F8EF7" : "#8892A4" }}>{site.score}</div>
                  ...
                  <div style={{ marginTop:12, background:"#1E2535", borderRadius:4, height:4 }}>
                    <div style={{ height:4, borderRadius:4, background: i===0 ? "linear-gradient(90deg,#4F8EF7,#A259FF)" : "#2A3350", width: site.score+"%" }} />
                  </div>
                </div>
              ))}
            </div>
                  ...
                  : panel.items.map(b => (
                    <div key={b.label} style={{ marginBottom:10 }}>
                      ...
                      <div style={{ background:"#1E2535", borderRadius:4, height:6 }}>
                        <div style={{ height:6, borderRadius:4, background:b.color, width: (b.count/history.length*100)+"%" }} />
                      </div>
                    </div>
                  ))
                }
